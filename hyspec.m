...
%
% ---=== Mouse interaction ===---
% - Spectra: Left, right and shift-left click to control channel 1, 2 and 3.
%           *Channel 1: single channel image
%           *Channel 1 and 2: two channel ratio image
%           *Channel 1 and 3: two colour RGB image
%           *Channel 1, 2 and 3: three channel RGB image
%            (Switch colour channels using Plot controls -> Switch RGB colours)
% - Histograms: Left and right click to crop histograms
%              *Make cropping permanent through Preprocess -> Apply histogram limits
%              *Combined with Selection -> Segmentation -> Histogram cut-off
%               left and right controls cut-off and ending segmentation.
%              *When displaying selections, click to make selections transparent.
% - Manipulation buttons: Select 'Zoom' or 'Pan' to focus on details.
% - Selection buttons: Selection of single/multiple pixels or polygon areas.
%
% ---=== Selection table ===---
% - Shows selected areas: position, size, name.
% - Changing names of selection affects spectrum legends and exported selections.
%   Change name to minus (-) to remove selection. Same names on two selections will
%   combine these. Prior weights (right-most column) is used for segmentation.
%
% ---=== Menues === ---
% File:            Load and save data, close GUI
% Dataset:         Select object (from Workspace) or copy image to Workspace.
% Preprocess:      Change image properties, e.g. size, or do spectral processing of image.
% Analysis:        Multivariate curve resolution.
% Selection:       Manage selections from image or perform image segmentation.
% Object analysis: Search for objects in images.
% Plot controls:   Colour handling and plot attributes plus image pop out (separate window).
%
function hyspec()
...
options.rgbCols = [0.8,0,0;0,0.8,0;0,0,1];
options.rgbOrder = 1; % Connection between channel and colour
options.rgbOrders = [1 2 3; 2 3 1; 3 1 2; 1 3 2; 2 1 3; 3 2 1];
currentDataset = [];
options.path  = cd;
options.openType = 1;
options.imNo  = 1;
...
options.wave2 = 0;
options.wave3 = 0;
% options.defaultRGB = [571, 545, 430]; % Wikipedia
% options.defaultRGB = [565, 535, 445]; % http://www.color-theory-phenomena.nl
% options.defaultRGB = [564, 533, 437]; % WikiLectures
% options.defaultRGB = [575, 535, 445]; % hyperphysics
options.defaultRGB = [564, 533, 437]; % Dowling 1987
options.waveOrder = [1 2 3]; % Order of colour histograms
options.clip  = false;
options.polys = [];
options.imMinMax  = [];
options.waveWidth = 0;
...
handles.popupmenu_ch2 = uicontrol(...
    'Parent',handles.fig,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[715 265 90 21],...
    'String','Ch.2',...
    'Style','popupmenu',...
    'Value',1,...
    'Tag','popupmenu_ch2');
handles.popupmenu_ch3 = uicontrol(...
    'Parent',handles.fig,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[810 265 90 21],...
    'String','Ch.3',...
    'Style','popupmenu',...
    'Value',1,...
    'Tag','popupmenu_ch3');
...
handles.uitable_select = uitable(...
    ...
    'ColumnEditable',[false true false false false true],...
    'ColumnName',{  'Sel.'; 'Name'; 'x'; 'y'; 'Px'; 'Prior' },...
    'ColumnWidth',{  30 70 40 40 40 35 },...
    'Data',[],...
    'BackgroundColor',[handles.BGColor;handles.BGColor-0.1],'ForegroundColor',handles.FGColor,...
    'Position',[675 325 260 100],...
    'RowName',[],...
    'UserData',[],...
    'Tag','uitable_select', ...
    'Visible', 'on');
...
handles.preproc{13} = uimenu(handles.preprocMenu,'Label','Rotate/flip image');
...
histMenu           = uimenu(handles.preprocMenu,'Label','Apply histogram limits');
handles.preproc{14} = uimenu(histMenu,'Label','To selected channel(s)');
handles.preproc{15} = uimenu(histMenu,'Label','To all channel(s)');
...
handles.regMenu = uimenu(handles.analysisMenu,'Label','Linear regression');
handles.regApply = uimenu(handles.regMenu,'Label','Apply regression vector');
handles.regShow = uimenu(handles.regMenu,'Label','View regression images', 'Enable','off');
...
handles.qvision    = uimenu(handles.segmentMenu,'Label','Qvision');
handles.segment{3} = uimenu(handles.qvision,'Label','Delete background(s)');
handles.segment{6} = uimenu(handles.qvision,'Label','Segmentation');
...
handles.segmentToImage = uimenu(handles.selectMenu,'Label','Promote segmentation to image','Enable','off');

handles.objectAnalyses = uimenu('Label','Object analyses');
handles.limitSizeRange = uimenu(handles.objectAnalyses,'Label','Limit object size range');
handles.identifySpots = uimenu(handles.objectAnalyses,'Label','Identify spots');
...
handles.rgbSwitch = uimenu(handles.plotMenu,'Label','Switch RGB channels');
handles.rgbDefault = uimenu(handles.plotMenu,'Label','Default RGB wavelengths');
handles.colormapMenu = uimenu(handles.plotMenu, 'Label','Colormap');
handles.axisEqual  = uimenu(handles.plotMenu,'Label','Equal axes','Checked','on','Separator','on');
...
set(handles.regShow,'Callback',@(hObject,eventdata)linPred_view(hObject,eventdata,handles));
set(handles.regApply,'Callback',@(hObject,eventdata)linPred(hObject,eventdata,handles));
...
set(handles.popupmenu_ch3,'Callback',@(hObject,eventdata)popupmenu_ch3_Callback(hObject,eventdata,handles))
for i=1:5
    set(handles.segment{i},'Callback',@(hObject,eventdata)segment(hObject,i,handles));
end
set(handles.segmentToImage,'Callback',@(hObject,eventdata)segmentToImage(hObject,eventdata,handles));
...
set(handles.rgbSwitch,'Callback',@(hObject,eventdata)rgbSwitching(hObject,eventdata,handles));
set(handles.rgbDefault,'Callback',@(hObject,eventdata)rgbDefaulting(hObject,eventdata,handles));
...
set(handles.limitSizeRange,'Callback',@(hObject,eventdata)limitSizeRange(hObject,eventdata,handles));
set(handles.identifySpots,'Callback',@(hObject,eventdata)identifySpots(hObject,eventdata,handles));


%% Set current colormap
function colorMap(hObject,which,handles) %#ok<*INUSL>
global options
if options.newGraphics
    nMaps = 12;
...
if get(handles.button1_2,'Value')==1  % Select polygon
    if ~isempty(currentDataset)
        ...
        if ~isempty(options.imp)
            ...
            options.meanSpectra = [options.meanSpectra; nanmean(spectra)];
            ...
            n_sel = size(data,1);
            if n_sel == 0
                set(handles.uitable_select,'Data',{1, '', round(mean(w2)),round(mean(w1)),size(spectra,1),1})
                options.whichSpectra = {ind2sub([px(1),px(2)],find(mask))};
            else
                last = max(cell2mat(data(:,1)));
                set(handles.uitable_select,'Data',[data;{last+1, '', round(mean(w2)),round(mean(w1)),size(spectra,1),1}])
...
global currentDataset options
filetypes = {'*.mat;*.MAT','MATLAB data file (*.mat)';
    '*.bin;*.bin','QVision bin file(s) (*.bin)';
    '*.fsm;*.fsm','PerkinElmer IR file(s) (*.fsm)';
    '*.mat;*.MAT','Unscrambler export for MATLAB (*.mat)';
    '*.hyspex;*.hyspex','HySpex file (*.hyspex)';
     '*.tif;*.tiff;*.jpg;*.jpeg','RGB images (*.tif(f), *.jp(e)g)'; ...
    '*.*',  'All Files (*.*)'};
typeOrder = [options.openType setdiff(1:6,options.openType)];
...
if index > 0
    ...
    if typeOrder(index) == 1 % MATLAB file
        options.openType = 1;
        for i = 1:length(filename)
    ...
    elseif typeOrder(index) == 2 % QVISION
        options.openType = 2;
...
%         end
    ...
    elseif typeOrder(index) == 3 % PerkinElmer IR file
        options.openType = 3;
        ...
        newDataset(handles)
        close(h)
    elseif typeOrder(index) == 5 % HySpex file
        options.openType = 5;
        nFile = length(filename);
        h = waitbar(0,['Importing image 1/' num2str(nFile)]);
        for i = 1:nFile
            waitbar((i-1)/nFile,h,['Importing image: ' num2str(i) '/' num2str(nFile)])
            [data,info] = enviread(filename{i}, [filename{i}(1:(end-6)) 'hdr']);
%             [data, xAxis, yAxis, zAxis] = fsmload(filename{i});
            dataset(i) = hyspec_object(data, 'x',1:size(data,1), 'y',1:size(data,2), 'v',cell2mat(eval(info.wavelength))); %#ok<AGROW>
            dataset(i).i = filename_orig{i}; %#ok<AGROW>
        end
        options.defaultRGB = dataset(1).v(cell2mat(eval(info.default_bands)));
        currentDataset = dataset;
        options.xlabs  = dataset(1).v;
        options.i = cell(1,i);
        if isempty(dataset(1).i)
    ...
    elseif typeOrder(index) == 6 % RGB image
        options.openType = 6;
        nFile = length(filename);
        for i = 1:nFile
            ...
            dataset(i) = hyspec_object(data, 'x',1:size(data,1), 'y',1:size(data,2), 'v',1:3); %#ok<AGROW>
            dataset(i).i = filename_orig{i}; %#ok<AGROW>
        end
        currentDataset = dataset;
        options.xlabs  = dataset(1).v;
...
%% Save data set
function saveCallback(hObject, eventdata, handles) %#ok<*INUSD>
global currentDataset options
...
if nargin==1
    ...
    options.wave2 = 0;
    options.wave3 = 0;
    if size(currentDataset(1).d,3) == 3
        options.wave2 = 2;
        options.wave3 = 3;
    end
    options.clip  = false;
    ...
    if imNo == options.imNo
        ...
        options.wave2 = 0;
        options.wave3 = 0;
        if size(currentDataset(imNo).d,3) == 3
            options.wave2 = 2;
            options.wave3 = 3;
        end
    end
...
elseif options.doMCR
    ...
    if size(tmp) > 4
        tmp = [tmp; [['PC 1';'PC 2';'PC 3';'PC 4';'PC 5'] repmat(' ', 5, size(tmp,2)-4)]];
    else
...
set(handles.popupmenu_ch3,'String',[['Ratio',repmat(' ', 1, max(0,size(tmp,2)-5))]; ...
    [repmat(' ',size(tmp,1),5-max(0,size(tmp,2))) tmp]],'Value',options.wave3+1)
...
if options.doPrediction
    theImage = options.predImages{options.imNo};
end
...
if options.wave2 > 0
    ...
    if options.wave3 == 0 && options.wave2 ~= options.wave1
        im = im./mean(theImage(:,:,ch),3);
    else
        im(:,:,2) = mean(theImage(:,:,ch),3);
    end
elseif options.wave2 < 0 && options.wave2 ~= options.wave1
    ...
    if options.wave3 == 0
        ...
        im(:,:,2) = options.scores(:,:,-options.wave2);
    end
end

% Extract third wavelength +/- bandwidth
if options.wave3 > 0
    ch = options.wave3;
    ch = ch + (-wi:1:wi);
    ch(ch<1) = []; ch(ch>length(options.xlabs)) = [];
    ...
    im(:,:,3) = mean(theImage(:,:,ch),3);
elseif options.wave3 < 0
    if isempty(options.scores) || size(options.scores,3) < -options.wave3
        [options.loadings,options.scores] = hyspec_pca(theImage,'ncomp',-options.wave3);
    end
    if options.wave2 == 0
        im(:,:,2) = NaN(size(im));
    end
    im(:,:,3) = options.scores(:,:,-options.wave3);
end
...
if length(size(im)) == 2
    ...
    image((im(:,:,options.rgbOrders(options.rgbOrder,:))-mm(1))./(mm(2)-mm(1)));
end
...
if ~popout 
    ...
    if options.doSegment
        ...
        if abs(m1-m2) < eps % Set midpoints of histogram bars
            s = m1;
            hi = hist(im(:),1);
        else
            s  = m1:(m2-m1)/(un-1):m2;
            if options.wave3 == 0
               hi = hist(im(:),s);
                ...
                i2 = im(:,:,2);
                hi(2,:) = hist(i2(:),s);
                if sum(hi(2,2:end)>0) == 0
                    hi(2,1) = 0;
                elseif sum(hi(2,1:end-1)>0) == 0
                    hi(2,end) = 0;
                end
                i3 = im(:,:,3);
                hi(3,:) = hist(i3(:),s);
    ...
    % Plot histogram bars
    if options.wave3 == 0 || options.doSegment
        ...
        % RGB histograms following order of last selections
        hold on
        for i = options.waveOrder
            hii = hi(i,:);
            patch([s1(:) s2(:) s2(:) s1(:)]', [zeros(un,1) zeros(un,1) hii(:) hii(:)]', 'b','EdgeColor','none','FaceColor', options.rgbCols(options.rgbOrders(options.rgbOrder,i),:))
            ss  = [s1;s2];
            sss = ss(:);
            hh  = [hii;hii];
            hhh = hh(:);
...
%              options.i = tmp.i;
    ...
    for i=1:length(currentDataset)
        currentDataset(i).d = double(currentDataset(i).d);
    end
...
%% Linear regression
function linPred(hObject, eventdata, handles)
objects = evalin('base','who');
handles.linPred1 = figure('Position',[250,250,261,144], 'WindowStyle','modal',...
    'Units','pixels', 'MenuBar','none', 'Resize','off', ...
    'Color',handles.BGColor);
handles.linPred1Text = uicontrol(...
    'Parent',handles.linPred1,...
    'Position',[20 106 111 14],...
    'String','Regression vector',...
    'Style','text',...
    'Tag','LinPred1text1');
handles.select_linPred1 = uicontrol(...
    'Parent',handles.linPred1,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[20 43 100 50],...
    'String',cellstr(objects),...
    'Style','popupmenu',...
    'Value',1,...
    'Tag','popupmenu_select_regvec');
handles.linPred1OK = uicontrol(...
    'Parent',handles.linPred1,...
    'Position',[121 13 100 31],...
    'String','OK',...
    'Tag','MCRpushbutton1');
handles.linPred1Cancel = uicontrol(...
    'Parent',handles.linPred1,...
    'Callback','close',...
    'Position',[20 13 100 31],...
    'String','Cancel',...
    'Tag','LinPredpushbutton1');
set(handles.linPred1Cancel,'Callback',@(hObject,eventdata)linPred1Close(hObject,eventdata,handles));
set(handles.linPred1OK,'Callback',@(hObject,eventdata)linPred1Callback(hObject,eventdata,handles));

function linPred1Close(hObject,eventdata,handles)
global options currentDataset
close
options.meanSpectra(1,:) = hyspec_mean(currentDataset(options.imNo).d);
newDataset(handles,options.imNo)

function linPred1Callback(hObject,eventdata,handles)
global options currentDataset
names = get(handles.select_linPred1,'String');
value = get(handles.select_linPred1,'Value');
betaName = names{value};
beta = evalin('base',betaName);
nIm  = length(currentDataset);
options.predImages = cell(nIm,1);
for i=1:nIm
    options.predImages{i,1} = predictImage(currentDataset(i).d, beta);
end
options.doPrediction = true;
set(handles.regShow,'Checked','on','Enable','on')
set(handles.emsc,'Checked','off')
set(handles.endSegment,'Checked','off')
close
newDataset(handles,options.imNo)
...
function linPred_view(hObject,eventdata,handles)
global options
if options.doPrediction
    set(handles.regShow,'Checked','off')
    options.doPrediction = false;
else
    set(handles.regShow,'Checked','on')
    options.doPrediction  = true;
end
set(handles.emsc,'Checked','off')
set(handles.endSegment,'Checked','off')
...
global options currentDataset %#ok<NUSED>
ncomp = str2num(get(handles.mcrEdit,'String')); %#ok<NASGU>
nn = zeros(1,2); um = zeros(1,2);
nn(1) = get(handles.mcrCheck1,'Value');
nn(2) = get(handles.mcrCheck2,'Value'); %#ok<NASGU>
um(1) = get(handles.mcrCheck3,'Value');
um(2) = get(handles.mcrCheck4,'Value'); %#ok<NASGU>
close
xL = get(handles.axes1,'XLim');
yL = get(handles.axes1,'YLim');
axes(handles.axes1)
...
text(xL(1)+range(xL)/2.01, yL(1)+range(yL)/2.01, 'Analysing', 'FontSize',30,...
    'HorizontalAlignment','center','Color',[1 1 1])
drawnow
% currentFinite = currentDataset;
% for i=1:length(currentFinite)
%     currentFinite(i).d = reshape(currentFinite(i).d, [length(currentFinite(i).x)*length(currentFinite(i).y),1,length(currentFinite(i).v)]);
%     currentFinite(i).d = currentFinite(i).d(isfinite(sum(currentFinite(i).d,3)),:,:);
% end
...
if ~isempty(currentDataset)
    ...
    switch which
        ...
        case 13
            rotateCallback(hObject, 1, handles);
        case 14
            histogramLimits(hObject, 1, handles);
        case 15
            histogramLimits(hObject, 2, handles);
    end
...
%% Apply histogram limits to displayed channel
function histogramLimits(hObject, eventdata, handles)
global currentDataset
global options
limits = options.imMinMax;
waves = [options.wave1 options.wave2 options.wave3];
if any(waves > size(currentDataset(1).d,3))
    errordlg('Cannot apply histogram limits from PCA')
elseif waves(1) > 0 && waves(2) > 0 && waves(3) == 0
    errordlg('Cannot apply histogram limits to channel ratios')
else
%     vec = @(x)x(:);
%     m1 = min(vec(currentDataset(options.imNo).d(:,:,waves(waves>0))));
%     m2 = max(vec(currentDataset(options.imNo).d(:,:,waves(waves>0))));
    for j=1:length(currentDataset)
        if eventdata == 1 % Single channel(s)

            for i = 1:3
                if waves(i) > 0
                  currentDataset(j).d(:,:,waves(i)) = trunkhist(currentDataset(j).d(:,:,waves(i)), limits);
%                     currentDataset(j).d(:,:,waves(i)) = imadjust(currentDataset(j).d(:,:,waves(i))./m2, limits./m2,[m1./m2,1]).*m2;
                end
            end
        else % All channels
            for i=1:size(currentDataset(j).d,3)
                currentDataset(j).d(:,:,i) = trunkhist(currentDataset(j).d(:,:,i), limits);
%                 currentDataset(j).d(:,:,waves(i)) = imadjust(currentDataset(j).d(:,:,i)./m2, limits./m2,[m1./m2,1]).*m2;
            end
        end
    end
end
...
if eventdata.Indices(2) == 2
    tmp = data{options.selectedSet,2}; % Name
    if strcmp(tmp,'-')
...
options.imMinMax = [];
options.waveOrder(options.waveOrder == 1) = [];
options.waveOrder = [options.waveOrder 1];
guidata(hObject, handles);
mainPlot(handles,false,false)
% plotSpectra(handles)
...
options.imMinMax = [];
options.waveOrder(options.waveOrder == 2) = [];
options.waveOrder = [options.waveOrder 2];guidata(hObject, handles);
mainPlot(handles,false,false)
% plotSpectra(handles)


%% Choose tertiary wavelength
function popupmenu_ch3_Callback(hObject, eventdata, handles)
global options
options.clip = false;
tmp = get(hObject,'Value');
if tmp-1 > size(options.xlabs,2)
    options.wave3 = size(options.xlabs,2)-tmp+1;
else
    options.wave3 = tmp-1;
end
options.imMinMax = [];
options.waveOrder(options.waveOrder == 3) = [];
options.waveOrder = [options.waveOrder 3];guidata(hObject, handles);
mainPlot(handles,false,false)
% plotSpectra(handles)
...
elseif sel == 2
    if ~options.reverse
        ...
        if options.wave2 ~= 0 && options.wave3 == 0  % Ratio
            im = im./currentDataset(options.imNo).d(:,:,options.wave2);
        elseif options.wave3 ~= 0
            if options.wave2 == 0
                im(:,:,2) = zeros(size(im));
            else
                im(:,:,2) = currentDataset(options.imNo).d(:,:,options.wave2);
            end
            im(:,:,3) = currentDataset(options.imNo).d(:,:,options.wave3);
        end
...
if options.wave1 > 0 && options.wave2 > 0 % Two wavelengths selected
    ...
    else % Ordinary wavelength plotting
        doPlot = true;
        if options.showMean
...
if options.doMCR
    ...
    for i = 2:size(options.MCR_S,1)
        tmp = [tmp ',''Comp. ' num2str(i) '''']; %#ok<AGROW>
    end
else
    if ind == 1
        tmp  = 'legend(''Image''';
...
if ~options.doMCR
    for i=1:(size(options.meanSpectra,1)-1)
        if size(data,1) >= i
            ...
            if isempty(data{i,2})
                tmp = [tmp '''Select. ' num2str(data{i,1}) '''']; %#ok<AGROW>
            else
                tmp = [tmp '''' data{i,2} '''']; %#ok<AGROW>
            end
        end
    end
end
...
if options.wave1 > 0
    plot((xlabs(options.wave1)-wi)*ones(1,2), get(gca,'YLim'),'-','Color',options.rgbCols(options.rgbOrders(options.rgbOrder,1),:))
    plot((xlabs(options.wave1)+wi)*ones(1,2), get(gca,'YLim'),'-','Color',options.rgbCols(options.rgbOrders(options.rgbOrder,1),:))
else
    tmp = [tmp ', ''PC ' num2str(-options.wave1) ''''];
end
if options.wave2 > 0
    plot((xlabs(options.wave2)-wi)*ones(1,2), get(gca,'YLim'),'-','Color',options.rgbCols(options.rgbOrders(options.rgbOrder,2),:))
    plot((xlabs(options.wave2)+wi)*ones(1,2), get(gca,'YLim'),'-','Color',options.rgbCols(options.rgbOrders(options.rgbOrder,2),:))
elseif options.wave2 < 0
    tmp = [tmp ', ''PC ' num2str(-options.wave2) ''''];
end
if options.wave3 > 0
    plot((xlabs(options.wave3)-wi)*ones(1,2), get(gca,'YLim'),'-','Color',options.rgbCols(options.rgbOrders(options.rgbOrder,3),:))
    plot((xlabs(options.wave3)+wi)*ones(1,2), get(gca,'YLim'),'-','Color',options.rgbCols(options.rgbOrders(options.rgbOrder,3),:))
end
...
elseif sel == 4
    options.wave3 = i;
    set(handles.popupmenu_ch3,'Value',options.wave3+1);
    uicontrol(handles.popupmenu_ch3)
    options.waveOrder(options.waveOrder == 3) = [];
    options.waveOrder = [options.waveOrder 3];
elseif sel == 3
    options.wave1 = 1;
...
if size(options.meanSpectra,1)>1 || i >= 3
    ...
    set(handles.segmentToImage,'Enable','on')
    axes(handles.axes1)
    xL = get(handles.axes1,'XLim');
    yL = get(handles.axes1,'YLim');
    axes(handles.axes1)
    ...
    if i == 1 % Nearest neighbour
        prior = cell2mat(data(:,6));
        if size(options.meanSpectra,1) == 2
            options.segmented = hyspec_segment(currentDataset, options.meanSpectra, 'prior', [1,prior]);
            options.segmentNames = {'Image';data{1,2}};
        else
            options.segmented = hyspec_segment(currentDataset, options.meanSpectra(2:end,:), 'prior', prior);
            options.segmentNames = data(:,2);
        end
    end
    if i == 2 % LDA
        if size(options.meanSpectra,1) == 2
    ...
    elseif i == 3 % Qvision, delete background
        if size(currentDataset(1).d,3) < 15
            error('Qvision algorithm needs 15 channels (or more)')
        else
            segments = hyspec_segment(currentDataset, [],'type',3);
            for k=1:length(currentDataset)
                for j=1:size(currentDataset(1).d,3)
                    tmp = currentDataset(k).d(:,:,j);
                    tmp(segments(k).d==1) = NaN;
                    currentDataset(k).d(:,:,j) = tmp;
                end
            end
        end
    elseif i == 4
...
%% Promote segment to image
function segmentToImage(hObject, eventdata, handles)
global options currentDataset
currentDataset = options.segmented;
options.doSegment = false;
options.imMinMax = [];
set(handles.endSegment,'Checked','off')
set(handles.addSegment,'Enable','off')
set(handles.segmentToImage,'Enable','off')
set(handles.emsc,'Checked','off')
set(handles.viewMCR,'Checked','off')
options.doMCR     = false;
options.doEMSC    = false;
...
options.xlabs     = 1;
options.polys     = [];
newDataset(handles,1)


%% Limit size range of objects
function limitSizeRange(hObject, eventdata, handles)
global currentDataset
n_dim = length(size(currentDataset.d));
if n_dim > 2
    errordlg('Only available for single channel, gray scale, or black/white images')
else
    levels = unique(currentDataset.d);
    n_channels = length(levels);
    if n_channels > 20
        errordlg('The procedure is not suited for images with many levels')
    else
        sizeRangeCallback(hObject,eventdata,handles)
    end
end

function sizeRangeCallback(hObject, eventdata, handles)
global options
handles.sizeRange = figure('Position',[250,250,280,120], 'WindowStyle','modal',...
    'Units','pixels', 'MenuBar','none', 'Resize','off', ...
    'Color',handles.BGColor);
handles.sizeRangeText1 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.sizeRange,...
    'Position',[20 94 75 16],...
    'String','Minimum number of pixels',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derText1');
handles.sizeRangeEdit1 = uicontrol(...
    'Parent',handles.sizeRange,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    ...
    'Style','edit',...
    'Tag','derEdit1');
handles.sizeRangeText2 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.sizeRange,...
    'Position',[105 94 75 16],...
    'String','Maximum number of pixels',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derText1');
handles.sizeRangeEdit2 = uicontrol(...
    'Parent',handles.sizeRange,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[105 70 51 22],...
    'String','1000',...
    'Style','edit',...
    'Tag','derEdit1');
handles.sizeRangeText3 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.sizeRange,...
    'Position',[170 94 75 16],...
    'String','Background',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derText3');
handles.select_background = uicontrol(...
    'Parent',handles.sizeRange,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[170 43 100 50],...
    'String',options.segmentNames,...
    'Style','popupmenu',...
    'Value',1,...
    'Tag','popupmenu_select_background');
handles.sizeRangeOK1 = uicontrol(...
    'Parent',handles.sizeRange,...
    'Units','pixels',...
    'Position',[90,20,70,30],...
    'String','OK',...
    'Tag','pushbutton1');
handles.sizeRangeCancel1 = uicontrol(...
    'Parent',handles.sizeRange,...
    'Units','pixels',...
    'Callback','close',...
    'Position',[20,20,70,30],...
...
set(handles.sizeRangeCancel1,'Callback',@(hObject,eventdata)sizeRangeClose(hObject,eventdata,handles));
set(handles.sizeRangeOK1,'Callback',@(hObject,eventdata)sizeRangePerformCallback(hObject,eventdata,handles));

%% Cancelling
function sizeRangeClose(hObject,eventdata,handles)
close
mainPlot(handles,false,false)

function sizeRangePerformCallback(hObject,eventdata,handles)
global options currentDataset
mini = str2num(get(handles.sizeRangeEdit1,'String'));
maxi = str2num(get(handles.sizeRangeEdit2,'String'));
background = get(handles.select_background,'Value');
close(handles.sizeRange)
xL = get(handles.axes1,'XLim');
yL = get(handles.axes1,'YLim');
axes(handles.axes1)
text(xL(1)+range(xL)/2, yL(1)+range(yL)/2, 'Preprocessing', 'FontSize',30,...
...
levels = unique(currentDataset.d);
n_channels = length(levels);
for i=setdiff(1:n_channels,background)
    CC = bwconncomp(currentDataset.d == i);
    lengths = cellfun(@length,CC.PixelIdxList);
    for j=1:length(lengths)
        if lengths(j) < mini || lengths(j) > maxi
            currentDataset.d(CC.PixelIdxList{j}) = background;
        end
    end
end
options.meanSpectra(1,:) = hyspec_mean(currentDataset(options.imNo).d);
...
%% Identify spots
function identifySpots(hObject, eventdata, handles)
global options
n_sel = length(options.selectedNames);
if n_sel == 0
    errordlg('Perform at least one selection')
else
    if n_sel > 20
        errordlg('The procedure is not suited for high numbers of selections')
    else
        identifySpotsCallback(hObject,eventdata,handles)
    end
end

function identifySpotsCallback(hObject, eventdata, handles)
global options
handles.identifySpots = figure('Position',[250,250,370,180], 'WindowStyle','modal',...
    'Units','pixels', 'MenuBar','none', 'Resize','off', 'Name', 'Spot identification', ...
    'Color',handles.BGColor);
handles.identifySpotsText3 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.identifySpots,...
    'Position',[20 144 75 16],...
    'String','Erode',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derText3');
handles.identifySpotsEdit3 = uicontrol(...
    'Parent',handles.identifySpots,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[20 120 51 22],...
    'String','4',...
    'Style','edit',...
    'Tag','derEdit3');
handles.identifySpotsText4 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.identifySpots,...
    'Position',[20 94 75 16],...
    'String','Dilate',...
    ...
    'HorizontalAlignment','left',...
    'Tag','derText4');
handles.identifySpotsEdit4 = uicontrol(...
    'Parent',handles.identifySpots,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[20 70 51 22],...
    'String','4',...
    'Style','edit',...
    'Tag','derEdit4');
handles.identifySpotsText1 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.identifySpots,...
    'Position',[95 144 80 16],...
    'String','Minimum size',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derText1');
handles.identifySpotsEdit1 = uicontrol(...
    'Parent',handles.identifySpots,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[95 120 51 22],...
    'String','200',...
    'Style','edit',...
    'Tag','derEdit1');
handles.identifySpotsText2 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.identifySpots,...
    'Position',[95 94 80 16],...
    'String','Maximum size',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derText1');
handles.identifySpotsEdit2 = uicontrol(...
    'Parent',handles.identifySpots,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[95 70 51 22],...
    'String','1000',...
    'Style','edit',...
    'Tag','derEdit1');
handles.identifySpotsTextB = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.identifySpots,...
    'Position',[180 124 75 16],...
    'String','Background',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derTextB');
handles.select_background = uicontrol(...
    'Parent',handles.identifySpots,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[180 73 100 50],...
    'String',[options.selectedNames, 'Remaining pixels'],...
    'Style','popupmenu',...
    'Value',length(options.selectedNames)+1,...
    'Tag','popupmenu_select_background');
handles.identifySpotsText5 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.identifySpots,...
    'Position',[300 144 75 16],...
    'String','Symmetry',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derText5');
handles.identifySpotsEdit5 = uicontrol(...
    'Parent',handles.identifySpots,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[300 120 51 22],...
    'String','2',...
    'Style','edit',...
    'Tag','derEdit5');
handles.identifySpotsText6 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.identifySpots,...
    'Position',[300 94 75 16],...
    'String','Fill',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','derText6');
handles.identifySpotsEdit6 = uicontrol(...
    'Parent',handles.identifySpots,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Position',[300 70 51 22],...
    'String','0.5',...
    'Style','edit',...
    'Tag','derEdit6');
handles.identifySpotsOK1 = uicontrol(...
    'Parent',handles.identifySpots,...
    'Units','pixels',...
    'Position',[90,20,70,30],...
    'String','OK',...
    'Tag','pushbutton1');
handles.identifySpotsCancel1 = uicontrol(...
    'Parent',handles.identifySpots,...
    'Units','pixels',...
    'Callback','close',...
    'Position',[20,20,70,30],...
...
set(handles.identifySpotsCancel1,'Callback',@(hObject,eventdata)identifySpotsClose(hObject,eventdata,handles));
set(handles.identifySpotsOK1,'Callback',@(hObject,eventdata)identifySpotsPerformCallback(hObject,eventdata,handles));

%% Cancelling
function identifySpotsClose(hObject,eventdata,handles)
close
mainPlot(handles,false,false)

function identifySpotsPerformCallback(hObject,eventdata,handles)
global options currentDataset
mini = str2num(get(handles.identifySpotsEdit1,'String'));
maxi = str2num(get(handles.identifySpotsEdit2,'String'));
erode  = str2num(get(handles.identifySpotsEdit3,'String'));
dilate = str2num(get(handles.identifySpotsEdit4,'String'));
background = get(handles.select_background,'Value');
symmetry = str2num(get(handles.identifySpotsEdit5,'String'));
fill     = str2num(get(handles.identifySpotsEdit6,'String'));
close(handles.identifySpots)
xL = get(handles.axes1,'XLim');
yL = get(handles.axes1,'YLim');
axes(handles.axes1)
text(xL(1)+range(xL)/2, yL(1)+range(yL)/2, 'Analyzing', 'FontSize',30,...
    'HorizontalAlignment','center')
text(xL(1)+range(xL)/2.01, yL(1)+range(yL)/2.01, 'Analyzing', 'FontSize',30,...
    'HorizontalAlignment','center','Color',[1 1 1])
drawnow

% Segmentation
h = waitbar(0,'Segmenting');
[x1,x2,p] = size(currentDataset(options.imNo).d);
n_sel = length(options.selectedNames);
data = get(handles.uitable_select,'Data');
prior = cell2mat(data(:,6));
if background == n_sel+1
    which_rest = setdiff(1:(x1*x2), unique(cell2mat(options.whichSpectra')'));
    Ivec = reshape(currentDataset(options.imNo).d,x1*x2,p);
    rest_mean = mean(Ivec(which_rest,:));
    tmpI = hyspec_segment(currentDataset, [options.meanSpectra(2:end,:);rest_mean], 'prior', [prior;1]);
    n_levels = n_sel+1;
else
    tmpI = hyspec_segment(currentDataset, options.meanSpectra(2:end,:), 'prior', prior);
    n_levels = n_sel;
end
tmpIlev = cell(length(currentDataset),n_levels);

% Erode
waitbar(0.2,h,'Eroding')
for i=1:length(currentDataset)
    k = 1;
    for j=setdiff(1:n_levels, background)
        tmpIlev{i,j} = imerode(tmpI(i).d==k, strel('disk',erode));
        k=k+1;
    end
end

% Dilate
waitbar(0.4,h,'Dilating')
for i=1:length(currentDataset)
    for j=setdiff(1:n_levels, background)
        tmpIlev{i,j} = imdilate(tmpIlev{i,j}, strel('disk',dilate));
    end
end

% Constrain sizes
% for I = 1:length(currentDataset)
waitbar(0.6,h,'Constraining')
I = options.imNo;
    imI = zeros(size(tmpI(I).d))+background;
    k = 1;
    for j=setdiff(1:n_levels, background)
        imI(tmpIlev{I,j}) = k;
        k=k+1;
    ...
    for i=setdiff(1:n_channels,background)
        CC = bwconncomp(imI == i);
        lengths = cellfun(@length,CC.PixelIdxList);
        sizes = regionprops(CC,'MajorAxisLength','MinorAxisLength','ConvexArea','Area');
        for j=1:length(lengths) 
            if lengths(j) < mini || lengths(j) > maxi || ...                  % Remove small, large 
                    sizes(j).MajorAxisLength > symmetry*sizes(j).MinorAxisLength ... % and highly unsymmetric
                    || sizes(j).MajorAxisLength*sizes(j).MinorAxisLength*3.14/4*fill > sizes(j).Area % and badly filled areas
                imI(CC.PixelIdxList{j}) = background;
                CC.PixelIdxList{j} = [];
            end
        end
    end
%     Inbwd = bwperim(imI==1);
%     m1 = min(currentDataset(I).d(:)); m2 = max(currentDataset(I).d(:));
%     overlay1 = imoverlay((currentDataset(I).d-m1)./(m2-m1), Inbwd, [1 .3 .3]);

    for i=1:size(currentDataset(I).d,3)
        cdi = currentDataset(I).d(:,:,i);
        cdi(imI ~= background) = NaN;
        currentDataset(I).d(:,:,i) = cdi;
    end        
% end
close(h)
options.meanSpectra(1,:) = hyspec_mean(currentDataset(options.imNo).d);
...
if ps == 1
    set(handles.axes1,              'Position', [40, 30, 520+p3,520+p4]);%p34 p34]);
elseif ps == 2
    p24 = (p34-p34/1.444)/2;
    set(handles.axes1,              'Position', [40+p24 30+p24 p34/1.444 p34/1.444]);
else
...
set(handles.popupmenu_ch1,      'Position', [p3+620 265 90 21]);
set(handles.popupmenu_ch2,      'Position', [p3+715 265 90 21]);
set(handles.popupmenu_ch3,      'Position', [p3+810 265 90 21]);
set(handles.edit_width,         'Position', [p3+930 264 31 22]);
%                                                 620 715 810 930
set(handles.text7,              'Position', [p3+963 261 31 22]);
set(handles.panel2,             'Position', [170    p4+580 400 70]);
%                                                  170 580 400 70
set(handles.uitable_select,     'Position', [p3+675 pos(4)/2 260 100]);
%                                               675 320 250 100
if too_small
...
for i=1:nseg
    ...
    if isempty(data)
        data = {1, ['Seg. ' num2str(i)], '', '', length(inds), 1};
    else
        data(nsel+i,:) = {max(cell2mat(data(:,1)))+1, ['Seg. ' num2str(i)], '', '', length(inds), 1};
    end
...
%% Rotate dialog
function rotateCallback(hObject, eventdata, handles)
handles.rotate1 = figure('Position',[250,250,200,120], 'WindowStyle','modal',...
    'Units','pixels', 'MenuBar','none', 'Resize','off', ...
    'Color',handles.BGColor);
handles.rotateText1 = uicontrol(...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    'Parent',handles.rotate1,...
    'Position',[20 90 135 20],...
    'String','Lossless rotation / flipping',...
    'Style','text',...
    'HorizontalAlignment','left',...
    'Tag','rotateText1');
handles.popupmenu_rotate = uicontrol(...
    'Parent',handles.rotate1,...
    'BackgroundColor',handles.BGColor,'ForegroundColor',handles.FGColor,...
    ...
    'Style','popupmenu',...
    'Value',1,...
    'Tag','popupmenu_rotateL');
tmp = [' 90 CW  ';
       '180 CW  ';
       '270 CW  ';
       'Flip L/R';
       'Flip U/D'];
set(handles.popupmenu_rotate,'String',tmp,'Value',1)

handles.rotateOK1 = uicontrol(...
    'Parent',handles.rotate1,...
    'Units','pixels',...
    'Position',[90,15,70,30],...
    'String','OK',...
    'Tag','rotateOK1');
handles.rotateCancel1 = uicontrol(...
    'Parent',handles.rotate1,...
    'Units','pixels',...
    'Callback','close',...
    ...
    'String','Cancel',...
    'Tag','rotateCancel1');
set(handles.rotateCancel1,'Callback',@(hObject,eventdata)rotateClose(hObject,eventdata,handles));
set(handles.rotateOK1,'Callback',@(hObject,eventdata)rotateRotateCallback(hObject,eventdata,handles));

%% Cancelling
function rotateClose(hObject,eventdata,handles)
close
mainPlot(handles,false,false)

%% Rotateing
function rotateRotateCallback(hObject,eventdata,handles)
global options currentDataset
rotate = get(handles.popupmenu_rotate,'Value');

% Rotering
im  = currentDataset(options.imNo);
imR = im;
if rotate == 1
    imR.d = permute(im.d,[2,1,3]);
    imR.d = imR.d(:,end:-1:1,:);
    imR.x = im.y;
    imR.y = im.x(end:-1:1);
elseif rotate == 2
...
currentDataset(options.imNo) = imR;
close
i = options.imNo;
newDataset(handles)
options.imNo = i;
...
axis(handles.axes1)
xl = get(gca,'XLim');
xl(1) = max(1,round(xl(1)));
xl(2) = min(size(currentDataset(options.imNo).d,2),round(xl(2)));
yl = get(gca,'YLim');
yl(1) = max(1,round(yl(1)));
yl(2) = min(size(currentDataset(options.imNo).d,1),round(yl(2)));
...
handles.popupmenu_cropL = uicontrol(...
    ...
    'Tag','popupmenu_cropL');
tmp = num2str((1:y)');
set(handles.popupmenu_cropL,'String',tmp,'Value',xl(1))
handles.popupmenu_cropR = uicontrol(...
    ...
    'Value',1,...
    'Tag','popupmenu_cropR');
set(handles.popupmenu_cropR,'String',tmp,'Value',xl(2))
...
handles.popupmenu_cropT = uicontrol(...
    ...
    'Tag','popupmenu_cropT');
tmp = num2str((1:x)');
set(handles.popupmenu_cropT,'String',tmp,'Value',yl(1))
handles.popupmenu_cropB = uicontrol(...
    ...
    'Value',1,...
    'Tag','popupmenu_cropB');
set(handles.popupmenu_cropB,'String',tmp,'Value',yl(2))
...
if ~isempty(constituent)
    ...
    for i=1:length(constNum)
        if constNum(i) > 0 % Constituent spectrum from selections
            ...
            eval(['tmpC = options.meanSpectra(' num2str(abs(constNum(i))+1) ',:);'])
            tmpC = vecOrth(ref',tmpC');
            eval('const = [const;tmpC''];')
%             tmpC = tmpC-mean(tmpC);
%             tmpC = tmpC./std(tmpC).*std(ref)+mean(ref);
%             eval('const = [const;ref-tmpC];')
        end
...
%% Switch RGB colours
function rgbSwitching(hObject,eventdata,handles)
global options
options.rgbOrder = mod(options.rgbOrder,6)+1;
mainPlot(handles,false,false)


%% Default RGB wavelengths
function rgbDefaulting(hObject,eventdata,handles)
global options
x = options.xlabs;
if length(x) == 3
    options.wave1 = 1;
    ...
    uicontrol(handles.popupmenu_ch3)
    options.rgbOrder = 1;
    options.waveOrder = [1 2 3];    
elseif max(options.defaultRGB) <= max(x) && min(options.defaultRGB) >= min(x)
    w1 = (x-options.defaultRGB(1)).^2;
    [~, options.wave1] = min(w1);
    set(handles.popupmenu_ch1,'Value',options.wave1);
    uicontrol(handles.popupmenu_ch1)
    w2 = (x-options.defaultRGB(2)).^2;
    [~, options.wave2] = min(w2);
    set(handles.popupmenu_ch2,'Value',options.wave2+1);
    uicontrol(handles.popupmenu_ch2)
    w3 = (x-options.defaultRGB(3)).^2;
    [~, options.wave3] = min(w3);
    set(handles.popupmenu_ch3,'Value',options.wave3+1);
    uicontrol(handles.popupmenu_ch3)
    options.rgbOrder = 1;
    options.waveOrder = [1 2 3];
else
    errordlg('Default RGB wavelengths are outside the available range.')
end
mainPlot(handles,false,false)


% %% Linear image adjust
% function X = linimadjust(X, from, to)
% X = (X-from(1)).*(range(to)./range(from)) + to(1);


%% Trunkate from histogram
function X = trunkhist(X, lims)
X(X<lims(1)) = lims(1);
X(X>lims(2)) = lims(2);


%% Pixel-wise linear prediction in image
function Yhat = predictImage(I, beta)
p = size(I,3);
if length(beta) == p
    Yhat = sum(bsxfun(@times,bsxfun(@minus,I,nanmean(nanmean(I,2),1)),reshape(beta,[1,1,p])),3);
elseif length(beta) == p+1
    Yhat = sum(bsxfun(@times,bsxfun(@minus,I,nanmean(nanmean(I,2),1)),reshape(beta(2:end),[1,1,p])),3)+beta(1);
%     Yhat = sum(bsxfun(@times,I,reshape(beta(2:end),[1,1,p])),3)+beta(1);
else
    disp('Regression vector has wrong size')
    Yhat = zeros(size(I));
end
