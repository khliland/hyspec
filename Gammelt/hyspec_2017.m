...
%   Change name to minus (-) to remove selection.
%
% ---=== Menues === ---
...
for i=1:nseg
    ...
    if isempty(data)
        data = {1, ['Seg. ' num2str(i)], '', '', length(inds)};
    else
...
global options
x = options.xlabs;
if max(options.defaultRGB) <= max(x) && min(options.defaultRGB) >= min(x)
