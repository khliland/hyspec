...
handles.preproc{6} = uimenu(handles.preprocMenu,'Label','Baseline correct','Enable','off');
...
if get(handles.button1_2,'Value')==1
    ...
    set(handles.button4_2,'Value',1)
    mainPlot(handles, false, false)
elseif get(handles.button2_2,'Value')==1
...
elseif get(handles.button3_2,'Value')==1
    options.mouse = 3;
    axes(handles.axes1)
    zoom on
else
...
% eval(['colormap(' options.colormap '(' num2str(min(256,length(unique(im(:))))) '))'])
% if ~options.doSegment
...
imagesc(im,mm)
if options.doSegment
%     mmSeg = [min(imSeg(:)) max(imSeg(:))];
%     if mmSeg(1)==mmSeg(2) % Handle "flat" images
%         mmSeg(1) = mmSeg(1)-eps;
%     end
...
if n > 0
    ...
    for i=1:n
        selected{i,1} = struct('Names',options.selectedNames{i}, ...
            ...
            'Polygons',options.polys{i}); %#ok<NASGU>
    end
...
set(handles.panel2,             'Position', [170    p4+585 360 50]);
