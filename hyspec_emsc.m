function [corrected, parameters, par_names, model] = hyspec_emsc(I, varargin)
%% EMSC for hyperspectral images 
% [corrected, parameters, par_names, model] = hyspec_emsc(I, varargin)
% 
% --= Compulsory argument =--
% I:
...
if isstruct(I)
    ...
    for j=1:i
        ...
        M = blockproc(I(j).d,block,fun_emsc, 'UseParallel', parallel);
        
        % Determine size and collect block calculations
        corrected(j).d  = M(:,:,1:p);
        parameters(j).d = M(:,:,(p+1):end);
        parameters(j).v = 1:size(parameters(j).d,3);
    end
else
    [r,c,~,~] = size(I);
...
% Orthogonalize model
[Q,~]           = qr(model(setdiff(1:terms,2),:)');
model(setdiff(1:terms,2),:) = Q(:,1:terms-1)';
% model(1,:)      = abs(model(1,:));
% model(1,:)     = ones(1,size(model,2));
